...
export default function StoryFinderPage() {
    ...
    return (
        <div className="flex flex-col gap-6 p-6 min-h-screen bg-zinc-950 text-zinc-100">
            ...
            {fetchError && (
                <div className="flex items-center justify-between gap-3 rounded-lg border border-rose-500/40 bg-rose-500/10 px-4 py-2 text-sm text-rose-300">
                    <span>{fetchError}</span>
                    <button
                        type="button"
                        onClick={fetchStories}
                        disabled={loading}
                        className="rounded-md border border-rose-400/40 bg-rose-500/10 px-3 py-1 text-xs font-semibold text-rose-200 transition-colors hover:bg-rose-500/20 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                        Coba Lagi
                    </button>
                </div>
